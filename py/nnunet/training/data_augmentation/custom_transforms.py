...
class RandomMotion(AbstractTransform):
    def __init__(self, degrees: float = 10, translation: float = 10, num_transforms: int = 2, image_interpoltion: str = 'linear') -> None:
        """add random MRI motion artifact
        reference: https://torchio.readthedocs.io/transforms/augmentation.html?highlight=random_motion#randommotion
        Args:
            degrees (float, optional): Rotation Range in degrees. Defaults to 10.
            translation (float, optional): Translation in mm of the simulated movements. Defaults to 10.
            num_transforms (int, optional): Number of simulated movements. Defaults to 2.
            image_interpoltion (str, optional): Image interpolation. Defaults to 'linear'.
        """
        self.degrees = degrees
        self.translation = translation
        self.n_transforms = num_transforms
        self.img_interpolation = image_interpoltion
        self.transform = tio.RandomMotion(self.degrees, self.translation, self.n_transforms, self.img_interpolation)
        print("Mona - torchio random motion")
...
class HistogramEqualization(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        if data is not None:
            for item in range(data.shape[0]):
                data[item] = exposure.equalize_hist(data[item])
        data_dict['data'] = data
        return data_dict
