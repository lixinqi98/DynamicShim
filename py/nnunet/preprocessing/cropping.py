...
def load_case_from_list_of_files(data_files, seg_file=None):
    ...
    if len((sitk.GetArrayFromImage(data_itk[0]).shape)) == 4:
        data_npy = sitk.GetArrayFromImage(data_itk[0])
        data_npy = np.transpose(data_npy, (3, 0, 1, 2))
        print(f"Mona:debug - permute the data into (C, X, Y, Z), shape {data_npy.shape}")
    else:
        data_npy = np.vstack([sitk.GetArrayFromImage(d)[None] for d in data_itk])
        print(f"Mona:debug - output data_npy shape {data_npy.shape}")
