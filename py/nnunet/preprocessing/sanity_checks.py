...
def resample_image_for_same_geometry(img_1: sitk.Image, img_2: sitk.Image):
    
    resample_img = sitk.Resample(img_2,
                             img_1.GetSize(),
                             sitk.Transform(), 
    ...
    for c in expected_train_identifiers:
        ...
        for i, img in enumerate(images_itk):
            ...
            if not same_geometry:
                ...
                label_itk = resample_image_for_same_geometry(img, label_itk)
                same_geometry = verify_same_geometry(img, label_itk)
                if same_geometry:
                    geometries_OK = True
                    print("Mona: The geometry is registered now")
            if nans_in_image:
